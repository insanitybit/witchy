...
| **Namespace ownership + authorization** (§8) | **Built.** Publish/promote/yank carry a bearer token (`COVEN_TOKEN`); the first publish claims a namespace (TOFU, `owners.json`), later publishes must match, and promote/yank are owner-only — so two-phase publish and separation-of-duties are not bypassable by anyone who can reach the server. |
| Remaining registry refinements (§8, §12) | **Modelled, not built.** Keyless **Sigstore/OIDC** identity (tokens are simple bearer), and full TUF **key separation** (per-namespace *delegated signing* keys — today ownership is enforced by token but all roles are signed by the one root key). |
