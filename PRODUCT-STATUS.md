...
| Generators and lazy iterators ([RFC-0130](rfcs/0130-generators-and-iterators.md)) | Supported preview | `Iter`, `gen fn`, `yield`, adapters, collection, diagnostics, and tooling are covered by the RFC acceptance matrix. Promoted generator shapes resume an owned frame, execute each segment once, retain bounded live state, and have linear pull overhead; fallback shapes do not inherit those effect or complexity guarantees. |
