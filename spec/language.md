...
and provides bounded generic algorithms (`cmp.max_of`, `cmp.sort`, ...);
`Eq`-bounded list membership/lookup live in `list` (`list.contains`,
`list.index_of`). `Show` (`import show`) renders.
