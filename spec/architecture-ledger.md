...
| `witchy-wir/wir_helpers/{mod.rs,bytes.rs,collections.rs,dict.rs,memory.rs,numeric.rs,registry.rs,strings.rs,strings/transform.rs}` | ~1,480 + ~270 + ~265 + ~1,160 + ~860 + ~520 + ~1,200 + ~545 + ~870 | EXTRACT | The dispatcher, memory/RC constructors, byte buffers, list access/extraction, dictionary lookup/read projections, numeric conversion and checked arithmetic, and string inspection/transformation have distinct owners with compatibility-preserving re-exports. Dictionary mutation and capacity management remain together in `dict.rs`; continue moving encoding/crypto and host-family helpers. |
