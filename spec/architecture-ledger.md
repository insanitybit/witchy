...
| `witchy-confinement` | none | KEEP | Own the target-neutral derived filesystem, network, Fetch-origin, and syscall-class policy without depending on compiler stages, Wasmtime, browsers, or operating-system providers. |
...
shrink during the runtime-kernel phase. Its dependency on
`witchy-confinement` is durable policy vocabulary, not a compiler-stage edge.
...
| Runtime enforcement TCB | the `witchy-runtime::runtime` kernel plus its `runtime::host::*` family registrars, `witchy-confinement` policy vocabulary, and Wasmtime; platform providers remain open | EXTRACT | Every capability host family registers through a task-shaped `host::*` registrar with private handlers. `Capabilities::confinement_policy` is the single concrete-grant derivation boundary; future providers consume its normalized output rather than reinterpreting grants. The Wasm kernel coordinates admission (`link_capability_imports`), grants, VM construction, resource limits, and execution. Next: add platform providers and inject the compiler-service interface so post-compilation enforcement drops its parser/type/WIR implementation dependencies. |
...
|---|---|---|---|
| Stage crate `pub mod` trees | Census-narrowed (Lane S, `state/agents/narrow-witchy-*-census.md`): syntax/caps/types/wir/interp/lower narrowed and merged (incl. `reachability`/`tagged`/`escape`/`lexer`/`optimize` mods), wir_helpers narrowed earlier; remaining pub surface is resolved out-of-crate consumers. | NARROW | witchy-syntax narrowed: 63 census reductions across the 25 non-linker modules plus the `source_check` proof-wrapper contract fix, then 6 further residual reductions (the caveat's name-collision survivors); AST fields stay pub. The former RFC-lane hotspots (typeck/traits/interpreter/codegen) are now internally decomposed into responsibility modules (see the hotspots ledger); narrowing their remaining pub surface follows the same resolved-call-site census method. |
