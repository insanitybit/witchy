...
| `witchy-wir/wir_helpers/{mod,bytes,collections,memory,registry,strings}.rs` | ~2,855 + ~270 + ~265 + ~860 + ~1,200 + ~540 | EXTRACT | The dispatcher, memory/RC constructors, byte buffers, list access/extraction, and string comparison/search/indexing have distinct owners with compatibility-preserving re-exports. Continue moving collection operations, allocating string transforms, numeric conversion, encoding/crypto, and host-family helpers. |
...
| WIR helper implementation and dependency metadata | EXTRACT | The typed registry, memory/RC constructors, list access/extraction, byte buffers, and string inspection/indexing are isolated in `registry.rs`, `memory.rs`, `collections.rs`, `bytes.rs`, and `strings.rs`. Continue moving remaining constructors into domain modules without duplicating the catalog; narrow public compatibility only in a separate census-backed slice. |
...
| B. WIR helper registry/domain extraction | `witchy-wir/src/wir_helpers/**` | A; registry, memory/RC, list access/extraction, byte-buffer, and string-inspection slices complete; remaining domains pending | medium | WIR tests, differential matrix, Wasm shard, adversarial diff review |
