...
| `witchy-interp/interpreter{.rs,/reflection.rs}` | ~6,900 + ~700 | EXTRACT | Syntax-reflection decoding, hole substitution, and origin reconstruction now have one function-only module with no evaluator-state access. Continue separating environment/value state, capability adapters, tail-call analysis, evaluator execution, and launch APIs. |
...
| G. Traits/interpreter/codegen contexts | one hotspot and new owner modules per slice | active semantics landed; interpreter reflection boundary complete | high | owning stage, differential/Wasm evidence, adversarial review |
