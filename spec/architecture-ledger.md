...
| Compiler services offered to trusted Witchy programs | `witchy-runtime::runtime::compiler` owns the `CompilerServices` interface, its Wasmtime adapters, and the registry-backed default; `witchy-runtime::native::compiler` still owns the implementation | EXTRACT | The adapters call the interface injected through `VmState` (the primary path injects from `Runtime`; worker spawns construct the default — the recorded residual). Next: move the implementation and the default wiring above the kernel and shed the parser/type/WIR implementation dependencies. Diagnostics are preserved byte for byte. |
