...
| Root `src/lib.rs` façade | Task-shaped browser entrypoints remain intentional. Twelve internal-only stage re-exports have been deleted (`analysis`, `doc`, `generators`, `grants`, `value`, `wir`, `wir_helpers`, `wir_opt`, `native`, `net`, `wir_encode`, `linker`), and repository callers now name their owning crates. The remaining compatibility exports have active consumers. | NARROW | Review each remaining export against an explicit browser/library contract, migrate internal callers first, and delete the old path only with resolved call-site evidence. |
