...
| `witchy-wir/wir_helpers/{mod.rs,bytes.rs,collections.rs,collections/operations.rs,dict.rs,memory.rs,numeric.rs,registry.rs,strings.rs,strings/transform.rs}` | ~865 + ~270 + ~270 + ~625 + ~1,160 + ~860 + ~520 + ~1,200 + ~545 + ~870 | EXTRACT | The dispatcher, memory/RC constructors, byte buffers, list access/extraction/view/mutation operations, dictionary lookup/read projections, numeric conversion and checked arithmetic, and string inspection/transformation have distinct owners with compatibility-preserving re-exports. Dictionary mutation and capacity management remain together in `dict.rs`; continue moving encoding/crypto and host-family helpers. |
...
| WIR helper implementation and dependency metadata | EXTRACT | The typed registry, memory/RC constructors, list access/extraction/view/mutation operations, byte buffers, string inspection/transformation, dictionary lookup/read projections, and numeric helpers are isolated in domain modules. Dictionary mutation and capacity management remain in `dict.rs`; continue moving remaining constructors without duplicating the catalog, and narrow public compatibility only in a separate census-backed slice. |
...
| B. WIR helper registry/domain extraction | `witchy-wir/src/wir_helpers/**` | A; registry, memory/RC, list access/extraction/view/mutation, byte-buffer, string inspection/transformation, dictionary lookup/read, and numeric slices complete; remaining domains pending | medium | WIR tests, differential matrix, Wasm shard, adversarial diff review |
