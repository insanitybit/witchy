...
| `witchy-wir/wir_helpers/{mod,bytes,collections,dict,memory,registry,strings}.rs` | ~2,855 + ~270 + ~265 + ~1,160 + ~860 + ~1,200 + ~540 | EXTRACT | The dispatcher, memory/RC constructors, byte buffers, list access/extraction, dictionary lookup/read projections, and string comparison/search/indexing have distinct owners with compatibility-preserving re-exports. Dictionary mutation and capacity management remain together in `dict.rs`; continue moving allocating string transforms, numeric conversion, encoding/crypto, and host-family helpers. |
...
| B. WIR helper registry/domain extraction | `witchy-wir/src/wir_helpers/**` | A; registry, memory/RC, list access/extraction, byte-buffer, string-inspection, and dictionary lookup/read slices complete; remaining domains pending | medium | WIR tests, differential matrix, Wasm shard, adversarial diff review |
