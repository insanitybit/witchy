...
| `witchy-runtime/runtime{.rs,/compiler.rs,/host/**}` | ~1,530 + ~105 + ~2,295 | EXTRACT | Compiler-service imports and the crypto, clock/env/rand, network, filesystem/exec, build-step, and VM-worker host families have distinct adapter/registrar owners; the kernel retains VM construction (vmstate_from_caps), grant admission, staging, and execution policy. Continue separating the core diagnostic/memory family from the kernel. |
