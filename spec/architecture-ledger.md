...
| `witchy-runtime/runtime{.rs,/compiler.rs,/host/**}` | ~2,000 + ~105 + ~1,805 | EXTRACT | Compiler-service imports and the crypto, clock/env/rand, network, filesystem/exec, and build-step host families have distinct adapter/registrar owners; the staging, serve-pool, and vm.par_map worker machinery stays kernel-side with the VM-worker family, consuming Dir authority through one typed filesystem entrypoint. Continue separating VM-worker and core diagnostic/memory families from grant admission and execution policy. |
