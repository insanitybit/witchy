...
pub fn transpile_module(m: &Module) -> Result<String, String> {
    ...
    // The Dict runtime helper is emitted only when the program actually uses one.
...
fn gen_fn(f: &Function) -> Result<String, String> {
    ...
    for p in &f.params {
        ...
        match rust_ty(ty) {
            // A no-op capability granted to main (Console -> unit) is bound as a
            // local; in any other function it's an ordinary `()` parameter that
            // can be passed along. `mut` lets a body reassign a value parameter.
            Some(rt) if is_main && rt == "()" => {
                cap_lets.push_str(&format!("    let {} = ();\n", p.name));
            }
