...
pub(crate) fn synthesize_anon_record_impls(
    ...
    records
        .into_iter()
        .filter(|(name, _)| !existing.contains(&(reflect_trait.clone(), name.clone())))
        .map(|(name, fields)| reflect_impl(name, fields, &reflect_trait))
        .collect()
}

fn reflect_impl(name: String, fields: Vec<String>, trait_name: &str) -> ImplDef {
