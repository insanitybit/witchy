...
fn grantable_capability_marker_parses() {
    ...
    let Item::Type(t) = &m.items[0] else {
    ...
    let Item::Type(t2) = &m2.items[0] else {
        panic!("expected a type")
    };
    assert!(
        t2.sealed && !t2.grantable,
        "an ordinary capability is not grantable"
    );
...
fn must_consume_marker_is_nominal_declaration_metadata() {
    ...
    assert!(declarations
        .iter()
        .all(|definition| definition.must_consume));
...
fn sealed_type_marker_parses() {
    ...
    assert!(
        !t2.sealed && !t2.is_capability,
        "an ordinary type is not sealed"
    );

    // A `capability` record is sealed AND a capability.
    let m3 = parse_module("capability Session:\n    token: String\n").unwrap();
    let Item::Type(t3) = &m3.items[0] else {
        panic!("expected a type")
    };
    assert!(
        t3.sealed && t3.is_capability,
        "a capability is a sealed capability"
    );
...
fn pub_only_marks_function_declarations() {
    for src in [
        ...
        assert!(
            err.message.contains("`pub` may only precede a function"),
            "{err}"
        );
    }
...
fn impl_trait_param_desugars_to_a_bound() {
    ...
    let Item::Function(g) = &m2.items[0] else {
        panic!()
    };
    assert!(g.bounds.iter().any(|(_, t, _)| t == "Show"));
    assert!(g
        .bounds
        .contains(&("a".to_string(), "Ord".to_string(), Vec::new())));
}
...
fn anonymous_union_type_syntax_is_tag_shaped() {
    ...
    assert!(
        lower.message.contains("must start with an uppercase"),
        "{lower}"
    );
    ...
    let empty_payload =
        parse_module("type Bad = .[Empty()]\n").expect_err("empty payload parens rejected");
    assert!(
        empty_payload.message.contains("empty payload parens"),
        "{empty_payload}"
    );
}
...
"#,
    ...
    let Stmt::Expr(Expr::AnonCtor { tag, args }) = &nullary[0] else {
        panic!(
            "expected nullary anonymous union injection, got {:?}",
            nullary[0]
        );
    };
    ...
    assert!(
        labeled.message.contains("takes positional payloads"),
        "{labeled}"
    );
}
...
"#,
    ...
    assert!(
        m.imports.contains(&"meta".to_string()),
        "quote expr imports meta"
    );
    let Item::Function(f) = &m.items[0] else {
        panic!("expected function");
    };
    let Stmt::Expr(Expr::Call { name, args }) = &f.body.stmts[0] else {
        panic!(
            "expected compiler-owned expression call, got {:?}",
            f.body.stmts[0]
        );
    };
    ...
    assert!(
        err.message
            .contains("`quote module:` is not implemented yet"),
        "{err}"
    );
}
...
"#,
    ...
    let Stmt::Expr(Expr::Call { name, args }) = &f.body.stmts[0] else {
        panic!(
            "expected compiler-owned expression-hole call, got {:?}",
            f.body.stmts[0]
        );
    };
...
fn quote_expr_holes_are_rejected_outside_quote_expr() {
    ...
    assert!(
        err.message.contains("only valid inside `quote expr:`"),
        "{err}"
    );
}
...
fn quote_type_owns_structural_and_borrowed_types() {
    ...
    let compositions = parse_module("fn f():\n    quote type:\n        .{..Base, y: String}\n")
    ...
    let references = parse_module("mode opt\n\nfn f():\n    quote type:\n        &'a mut String\n")
...
fn function_types_retain_parameter_conventions() {
    let module =
        parse_module("fn use(f: fn(var Int, let String, own Bytes, Bool) -> Int):\n    0\n")
...
"#,
    ...
    assert_eq!(
        holes,
        &vec![Expr::Var("ok".into()), Expr::Var("err".into())]
    );
}
...
"#,
    ...
    assert_eq!(
        &args[0],
        &Expr::Str(m.compiler_block_syntax[0].handle.clone())
    );
    assert_eq!(
        &args[1..],
        &[
...
"#,
    ...
    let Stmt::Expr(Expr::Call { name, args }) = &f.body.stmts[0] else {
        panic!(
            "expected compiler-owned item call, got {:?}",
            f.body.stmts[0]
        );
    };
...
"#,
    ...
    let Stmt::Expr(Expr::Call { name, args }) = &f.body.stmts[0] else {
        panic!(
            "expected compiler-owned item-hole call, got {:?}",
            f.body.stmts[0]
        );
    };
...
fn var_receiver_round_trips_through_fmt() {
    ...
    assert!(
        out.contains("var xs: List(a)"),
        "var receiver must survive fmt: {out}"
    );
    assert_eq!(
...
fn borrowed_param_and_result_views_parse() {
    ...
    assert_eq!(
        f.params[0].ty, want,
        "`let('a) String` param is a borrowed view"
    );
...
fn explicit_reference_types_parse_and_retain_access_kind() {
    ...
    let Item::Function(read) = &module.items[0] else {
        panic!("read function")
    };
    assert!(matches!(
        read.params[0].ty,
    ...
    let Item::Function(edit) = &module.items[1] else {
        panic!("edit function")
    };
    assert!(matches!(
        edit.params[0].ty,
...
fn explicit_borrow_and_deref_expressions_parse() {
    ...
    let Stmt::Let { value, .. } = &main.body.stmts[1] else {
        panic!("view binding")
    };
    assert!(matches!(
        value,
    ...
    let Stmt::Let { value, .. } = &main.body.stmts[2] else {
        panic!("reborrow binding")
    };
    assert!(matches!(
        value,
...
fn borrowed_nominal_parameters_and_arguments_keep_their_kinds() {
    ...
    let Item::Type(definition) = &module.items[0] else {
        panic!("expected type")
    };
...
fn borrowed_single_variant_positional_nominal_parses() {
    let module = parse_module("mode opt\n\ntype Span('a):\n    Span(View(Bytes, 'a), Int, Int)\n")
...
fn type_aliases_reject_lifetime_parameters_at_the_declaration() {
    ...
    assert!(
        error
            .message
            .contains("only supported on a nominal type declaration")
            && error
                .message
                .contains("type aliases accept ordinary type parameters only"),
        "{error}"
    );
}
...
fn dyn_trait_parses_in_every_type_position() {
    ...
    let m = parse_module(src)
        .expect("dyn parses in alias, generic-arg, param, tuple, fn-type, and return positions");
    let Item::TypeAlias { ty, .. } = &m.items[0] else {
...
fn as_dyn_trait_parses_as_an_explicit_cast() {
    ...
    let crate::ast::Stmt::Let { value, .. } = &body[0] else {
        panic!("expected let")
    };
    let crate::ast::Expr::As { ty, .. } = value else {
        panic!("expected as-cast")
    };
    assert_eq!(ty, &Type::Dyn("Render".into(), vec![]));
}
...
fn dyn_with_empty_argument_list_is_a_precise_parse_error() {
    ...
    assert!(
        err.message
            .contains("empty trait-argument list is malformed"),
        "got: {}",
        err.message
    );
}
...
#[test]
fn slice_and_str_reference_types_parse_and_format() {
    let source = "mode opt\n\nfn process(s: &'a str, buf: &'b [Int], mut_slice: &'c mut [String]) -> &'a str:\n    s\n";
    let module = parse_module(source).expect("slice and str reference types should parse");
    let Item::Function(function) = &module.items[0] else {
        panic!("expected function");
    };
    assert_eq!(
        ...
        Some(Type::Qualified(
            ...
            Box::new(Type::Named("str".into(), Vec::new()))
        ))
    );
    assert_eq!(
        function.params[1].ty,
        Some(Type::Qualified(
            TypeQual::Borrow("b".into()),
            Box::new(Type::Slice(Box::new(Type::Named("Int".into(), Vec::new()))))
        ))
    );
    assert_eq!(
        function.params[2].ty,
        Some(Type::Qualified(
            TypeQual::BorrowMut("c".into()),
            Box::new(Type::Slice(Box::new(Type::Named(
                "String".into(),
                Vec::new()
            ))))
        ))
    ...
    assert_eq!(
        crate::format::type_str(function.params[1].ty.as_ref().unwrap()),
        "&'b [Int]"
    );
    assert_eq!(
        crate::format::type_str(function.params[2].ty.as_ref().unwrap()),
        "&'c mut [String]"
    );
}
