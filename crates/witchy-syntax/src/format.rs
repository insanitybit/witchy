    ...
    match e {
        ...
        Expr::Call { name, args } => {
            ...
            if name == crate::intrinsics::COMPILER_QUOTE_PATTERN
                && let [Expr::Str(_handle), Expr::Str(source)] = args.as_slice()
            {
                return format!("meta.pattern_join([{}], [])", string_lit(source));
            }
            if name == crate::intrinsics::COMPILER_QUOTE_PATTERN_HOLES
                && let [Expr::Str(_handle), parts, holes] = args.as_slice()
            {
                return format!("meta.pattern_join({}, {})", expr(parts), expr(holes));
            }
...
fn canon_module(m: &mut Module) {
    ...
    for syntax in &mut m.compiler_expr_syntax {
        syntax.handle.clear();
        syntax.definition_line = 0;
    ...
    for syntax in &mut m.compiler_pattern_syntax {
        syntax.handle.clear();
        syntax.definition_line = 0;
    }
    for syntax in &mut m.compiler_stmt_syntax {
        syntax.handle.clear();
        syntax.definition_line = 0;
        canon_stmt(&mut syntax.stmt);
    }
    for syntax in &mut m.compiler_block_syntax {
        syntax.handle.clear();
        syntax.definition_line = 0;
        canon_block(&mut syntax.block);
    }
...
fn canon_expr(e: &mut Expr) {
    if let Expr::Call { name, args } = e {
        // Compiler syntax handles are opaque lookup keys, not source semantics.
        // Formatting reconstructs them from canonical source and parse context;
        // implicit imports may become explicit, so compare the authoritative
        // payload tables and projections while ignoring the key bytes.
        if matches!(
            name.as_str(),
