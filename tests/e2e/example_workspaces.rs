...
fn run_workspace(name: &str, rune: &str, check: impl FnOnce(&str)) {
    let work = lift_example(name);
    let out = pm_fe(&work, &["run", rune]);
...
fn example_ledger_workspace_runs_with_async_and_a_path_dependency() {
    run_workspace("ledger", "ledger", |s| {
        ...
        assert!(application_output(s).ends_with("deposit  $0.99  -> balance $12.24"), "final balance wrong: {s}");
    });
}
...
fn example_wordfreq_workspace_ranks_words() {
    run_workspace("wordfreq", "wordfreq", |s| {
        let collapsed = s.split_whitespace().collect::<Vec<_>>().join(" ");
        assert!(collapsed.contains("Top words: the 6 fox 4 dog 3 quick 3 brown 2"), "ranking wrong: {collapsed}");
    });
}
