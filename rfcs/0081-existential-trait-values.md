...
created: 2026-07-12
superseded-by:
tracking: "slice 1 (type identity and safety) implemented: dyn Trait(args…) and dyn module.Trait(args…) parse/format in every type position; non-ambient trait declarations, impl heads, bounds, supertraits, and existential heads resolve to one module-qualified declaration identity; same-spelled imported traits require qualification; aliases and imports preserve identity; the six existential-safety rules and transitive capability-payload rejection are enforced; and every dyn-mentioning program fails with one feature-stage diagnostic before either backend lowers. Slice 2 metadata substrate is implemented: closed, type-resolved construction requests feed one backend-neutral witness plan; it assigns deterministic IDs, instantiates generic impl templates, checks conditional impl bounds, and records concrete payload types plus convention-bearing monomorphic adapter slots using the existing specialization symbols. A concrete-independent layout now linearizes the static existential method surface and is the sole slot-order input to every witness, so a dyn method call can resolve its numeric slot without a construction site or concrete type. A preserved witness catalog carries the complete resolved impl universe across trait lowering, including the same compiler-synthesized anonymous-union impls used by ordinary dispatch. Existential preparation runs only on the final trait-lowered, monomorphic TypedModule: expected-type checking records exact concrete-to-dyn requests at annotations, arguments, returns, assignments, aggregate slots, constructor fields, and explicit casts; each request selects a witness and rewrites in post-order to a source-unspellable pack carrying the resolved existential type and witness ID. Unresolved requests and missing witnesses fail loudly. Preparing earlier is rejected, and generic substitution rejects a pre-existing pack rather than retaining a stale witness ID. Every frontend, ownership, reachability, and lowering traversal preserves the pack or treats its allocation effects conservatively. The compiled backend now consumes the final witness plan, reserves a fixed `{structref payload, i32 witness}` envelope, and emits one immutable concrete GC payload box per closed witness using the payload's real scalar or reference WIR kind. The public stage gate remains closed: authenticated dispatch lowering with loan integration, adapter envelopes, and execution in both backends are blocking runtime work. Reflection metadata uses a placeholder opaque meta.TNamed head until owned existential values add meta.TDyn. Slices 3-5 (owned values and dispatch, receiver completion, authority/tooling closure) remain incomplete"
related:
...
A `var dyn Trait` argument is invariant: the caller place must already have the
exact existential type. It cannot implicitly erase a concrete caller place,
because the callee may replace the parameter with a different concrete witness
that cannot be written back into that place. Read-only and `own` arguments use
the ordinary directed conversion rule.

`??` does not implicitly map an `Option(T)` or `Result(T, E)` payload. Its
payload and fallback retain the ordinary same-type rule; use an explicitly
existential container or convert both paths explicitly when their concrete types
differ. This keeps evaluation and allocation visible instead of hiding a
container traversal inside coercion.
