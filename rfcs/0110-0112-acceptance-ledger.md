...
| 2 | **WASM PROVEN / INTERPRETER DEBT** | Compiled-Wasm aggregate carriers construct, copy, destructure, project, return, and preserve owner roots through linked/generic boundaries; erasing calls remain rejected. Debt: `shared_reference_tuple_preserves_each_owner_root_on_both_backends`; missing interpreter tuple construction/copy/destructure/projection carrier; converge when the aggregate carrier ABI is frozen. |
...
| 5 | **WASM PROVEN / INTERPRETER DEBT** | Function-value lifetime relations, nominal owner positions, and the unified access signature are checked together in callable and fixed-nominal matrices; compiled Wasm proves direct function-value and closure reference returns. Debt: `function_value_shared_reference_return_preserves_the_runtime_place_on_wasm`, `closure_shared_reference_return_preserves_the_runtime_place_on_wasm`, `function_value_exclusive_reference_return_writes_the_owner_on_wasm`, and `function_value_mutable_to_shared_reborrow_preserves_the_owner_on_wasm`; missing interpreter callable-carrier ABI; converge when callable reference arguments/results and reborrows are frozen. |
...
| 8 | **WASM PROVEN / INTERPRETER DEBT** | Compiled-Wasm fixtures prove root balance for explicit return, branch return, loop exit, and `?` early return. Debt: the return-root fixture family has no stable interpreter carrier ABI yet; converge when `ReferenceKind` return and reborrow ABI is frozen, then add named interpreter parity before this row becomes `PROVEN`. |
| 9 | **WASM PROVEN / INTERPRETER DEBT** | Compiled-Wasm direct reference-list fixtures prove construction, return, indexing, and selected exclusive writes. Debt: `shared_reference_list_preserves_each_owner_root_on_both_backends` and `exclusive_reference_list_projection_writes_the_selected_owner_on_both_backends`; missing interpreter list carrier/index/write boundary; converge when the list/direct-place write ABI is frozen. |
...
`WASM PROVEN / INTERPRETER DEBT` is likewise not `PROVEN`: it records an
executable experimental opt-mode floor only, and blocks RFC completion and exit
from experimental opt mode until the named interpreter boundary converges.
