...
| 2 | **PROVEN** | `example_tests::rfc0122_references::shared_reference_tuple_preserves_each_owner_root_on_both_backends` proves tuple construction, return, local binding, copy, destructure, and field projection preserve both owner roots on interpreter and compiled Wasm; erasing calls remain rejected. |
...
| 5 | **PROVEN** | Function-value lifetime relations, nominal owner positions, and the unified access signature are checked together in callable and fixed-nominal matrices. `example_tests::rfc0122_references` proves shared function-value and closure returns, exclusive function-value writes, and function-value mutable-to-shared reborrows on interpreter and compiled Wasm. |
...
| 8 | **PROVEN** | `example_tests::rfc0122_control_flow::reference_return_control_flow_preserves_the_runtime_place_on_both_backends` proves explicit return, branch return, loop exit, and `?` early-return paths preserve their reference owners on interpreter and compiled Wasm. The existing lowering root-balance fixtures retain structural coverage of the corresponding retain/release paths. |
| 9 | **PROVEN** | `example_tests::rfc0122_references::shared_reference_list_preserves_each_owner_root_on_both_backends` and `exclusive_reference_list_projection_writes_the_selected_owner_on_both_backends` prove direct reference-list construction, return, indexing, and selected exclusive writes on interpreter and compiled Wasm. |
