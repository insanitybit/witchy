...
| 9 | **PARTIAL** | Boundary re-own and ownership-token repair now have real, lever-invariant runtime counters (`__witchy_boundary_reown_copies` / `__witchy_ownership_token_repairs`, proven exact = 1 repaired / 0 accepted / 0 dead-branch). Indirect ownership-envelope calls and destination forwarding also have real counters. REMAINING: the direct-storage-access counter is still a placeholder (fires only once direct-storage `var` lowering lands, steps 8-10 of rfcs/impl-plans/0110-execution-plan.md). |
