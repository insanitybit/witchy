...
| 2 | **PARTIAL** | Opt-mode analysis now checks BOTH `var unique` and `own unique` call shapes at every call site (`CallOwnershipFact::unique_params_to_check` unions the write-back and consuming axes; `own_unique_consuming_param_is_checked_and_fresh_values_pass` proves opt-rejects-aliased / normal-accepts / fresh-owner-passes). The source-coordinate repair set exists (`analysis::module_boundary_repairs`, keyed `(function, line, callee, arg_index)`). REMAINING: the deterministic boundary-reown counter is declared but not yet fired at the codegen zero-token arms, so the "exactly one repair for the paired normal-mode case" half of criterion 9 is unproven (step 5 of rfcs/impl-plans/0110-execution-plan.md). |
