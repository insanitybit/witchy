...
| 9 | **PARTIAL** | Direct `List(B('a))` literals publish indexed owner-root contributions; `list.at` transfers the selected roots (or conservatively all roots for a dynamic index), and `for` binders keep the list companions live. Type and compiled-Wasm fixtures prove construction, read, and traversal. List copy, overwrite, drop, nested containers, and relation-erasing boundary coverage remain absent. |
