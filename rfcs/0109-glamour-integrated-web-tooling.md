...
updated: 2026-08-03
superseded-by:
tracking: "Design accepted; no open questions. (2026-08-04 triage) Of three open rows, two are BLOCKED-EXTERNAL — build-authenticated mount grants need a deployed-header doctor run + the approved WebAuthn relying-party exchange (ACCEPTANCE.md:195), and cold-mobile evidence needs controlled measurement against the approved reference deployment (:196). ONE row is IN-SANDBOX but DEEP: the incremental compiler (:134) — the dev watcher's fingerprinting/affected-module/parsed-module cache is done, but per-module checked-IR + codegen reuse (src/commands/web/dev.rs, witchy-types/src/pipeline.rs, witchy-lower/src/codegen/*) plus local incremental-timing thresholds remain, a multi-session codegen track (not one bounded slice). RFC-0107 Phase 4. Web dev loop, safe hot swap, static delivery, and deterministic rebuild are largely implemented and proven. Remaining before implemented: per-module incremental checked-IR/codegen reuse (RFC-0107-ACCEPTANCE.md row 134, still whole-unit relink), aggregate-model hot-swap serializer (rows 57-60), and the `witchy dev` static-delivery path (src/commands/web.rs:1005-1011 errors 'not implemented yet')."
predecessors:
