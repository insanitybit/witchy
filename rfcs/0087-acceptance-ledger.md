...
| 12 | **PROVEN** | Current master snapshot-tests a compiler/type-resolved census over 283 Witchy files and 190 README/spec/book blocks (473 checked entries): mechanical self-reassignment, immutable `var` arguments, and temporary `var` arguments are all zero. |
...
| 16 | **PROVEN** | The stable census records 38 entry-source `var` declarations, 662 resolved `var` calls, nine intentional expression-position auxiliary-result uses, 17 intentional statement-position auxiliary-result discards, and zero compiler rejections after RFC-0005 typed closure environments landed. |
