...
| 21 migration command | evidence/migration | partial | `witchy migrate references` rewrites proven direct local typed owner calls and reports ambiguous sites without writing them (`84d9a0da`). It resolves imported signatures before rewriting (`rfc0122_reference_migration_rewrites_resolved_imported_calls`); overload resolution, a repository rewrite report, and a legacy-free census remain. |
