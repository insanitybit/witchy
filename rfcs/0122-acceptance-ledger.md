...
| 14 owned qualifiers | checker/runtime/wasm | partial | `loans_tests::exclusive_reference_signature_retains_its_affine_contract` covers accepted `own unique &'a mut T`, rejects both frozen/unique orders and frozen reference targets, and rejects escaping `local unique &'a mut T`; `exclusive_handle_qualifiers_survive_aggregate_and_callable_positions` extends that through tuple and function-value positions; `unique_exclusive_reference_return_writes_the_owner_on_wasm` proves a consumed unique handle retains its writable carrier through return. The remaining qualifier matrix is still open. |
