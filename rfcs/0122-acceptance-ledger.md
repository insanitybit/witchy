...
| 18 aggregate affine roots | checker/runtime | partial | shared aggregate roots and `exclusive_reference_aggregates_move_once_and_destructure_without_copying` now prove an aggregate built from `&mut` borrows moves into one destructuring use and rejects recovery through the old aggregate spelling; list iteration and the remaining affine matrix remain |
