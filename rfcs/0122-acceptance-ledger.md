...
| 18 aggregate affine roots | checker/runtime | partial | `exclusive_reference_aggregates_move_once_and_destructure_without_copying` proves an aggregate built from `&mut` borrows moves into one destructuring use and rejects recovery through the old aggregate spelling. `exclusive_reference_list_iteration_writes_each_owner_on_wasm` proves returned-list iteration and writes through the compiled carrier; `loans_tests::exclusive_reference_loop_elements_are_affine` proves a loop element is moved, not copied. Interpreter loop-element reference binding is a debt until the aggregate/list carrier ABI freezes. The remaining affine matrix is open. |
