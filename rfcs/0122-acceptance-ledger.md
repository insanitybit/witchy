...
| 18 aggregate affine roots | `shared_reference_tuple_preserves_each_owner_root_on_both_backends`, `shared_reference_list_preserves_each_owner_root_on_both_backends`, `shared_reference_list_return_copy_and_projection_work_on_wasm`, `exclusive_reference_list_projection_writes_the_selected_owner_on_both_backends`, `exclusive_reference_list_move_then_projection_writes_the_selected_owner_on_wasm`, `exclusive_reference_list_iteration_writes_each_owner_on_wasm`, `aggregate_reference_telemetry_corpus_pins_schema_and_copy_parity` | list return/copy/index carrier in the interpreter; loop element binding for a returned `List(&mut T)`; telemetry execution currently has no interpreter aggregate carrier | aggregate/list carrier ABI frozen | direct construction/projection fixtures converged; `shared_reference_list_return_copy_and_projection_work_on_wasm`, moved-list projection, list iteration, and `aggregate_reference_telemetry_corpus_pins_schema_and_copy_parity` are `WASM PROVEN / INTERPRETER DEBT` |
