...
| 18 aggregate affine roots | `shared_reference_tuple_preserves_each_owner_root_on_both_backends`, `shared_reference_list_preserves_each_owner_root_on_both_backends`, `shared_reference_list_return_copy_and_projection_work_on_wasm`, `shared_reference_nominal_aggregate_return_copy_and_projection_work_on_wasm`, `exclusive_reference_list_projection_writes_the_selected_owner_on_both_backends`, `exclusive_reference_list_move_then_projection_writes_the_selected_owner_on_wasm`, `exclusive_reference_list_iteration_writes_each_owner_on_wasm`, `aggregate_reference_telemetry_corpus_pins_schema_and_copy_parity` | list/nominal aggregate return/copy/projection carrier in the interpreter; loop element binding for a returned `List(&mut T)`; telemetry execution currently has no interpreter aggregate carrier | aggregate/list carrier ABI frozen | direct construction/projection fixtures converged; `shared_reference_list_return_copy_and_projection_work_on_wasm`, nominal aggregate transport, moved-list projection, list iteration, and `aggregate_reference_telemetry_corpus_pins_schema_and_copy_parity` are `WASM PROVEN / INTERPRETER DEBT` |
...
| 2 conventional normal calls | boundary | partial | `rfc0122_normal_callers_use_conventional_opt_access_on_both_backends` and `rfc0122_normal_to_opt_unique_calls_preserve_value_semantics` prove reference-free `let`/`var`/`own` calls, while `BoundaryEntrySelection` derives proven/repair from the checked access graph and lowering consumes that selection; generated adapter ABI and normal-to-opt repair parity remain |
...
| 14 owned qualifiers | checker/runtime/wasm | partial | `loans_tests::exclusive_reference_signature_retains_its_affine_contract` covers accepted `own unique &'a mut T`, rejects both frozen/unique orders and frozen reference targets, and rejects escaping `local unique &'a mut T`; `exclusive_handle_qualifiers_survive_aggregate_and_callable_positions` extends that through tuple and function-value positions; `unique_exclusive_reference_return_writes_the_owner_on_wasm` and `unique_exclusive_reference_function_value_return_writes_the_owner_on_wasm` prove a consumed unique handle retains its writable carrier through direct and indirect return on compiled Wasm. The function-value fixture carries interpreter callable-carrier debt until the `ReferenceKind` indirect ABI freezes; the remaining qualifier matrix is still open. |
...
| 18 aggregate affine roots | checker/runtime | partial | `exclusive_reference_aggregates_move_once_and_destructure_without_copying` proves an aggregate built from `&mut` borrows moves into one destructuring use and rejects recovery through the old aggregate spelling. `shared_reference_list_return_copy_and_projection_work_on_wasm` proves the compiled list carrier preserves return, local binding, reference copy, and indexed projection; `shared_reference_nominal_aggregate_return_copy_and_projection_work_on_wasm` extends that Wasm carrier through a nominal aggregate and field projection; both interpreter boundaries are explicit debt until the aggregate/list carrier ABI freezes. `exclusive_reference_list_move_then_projection_writes_the_selected_owner_on_wasm`, `exclusive_reference_list_iteration_writes_each_owner_on_wasm`, and `exclusive_reference_list_iteration_reborrows_and_resumes_each_element_on_wasm` prove returned-list move, projection, iteration, reborrow, parent resumption, and writes through the compiled carrier; `aggregate_reference_telemetry_corpus_pins_schema_and_copy_parity` pins the same Wasm carrier's optimized/forced-copy facts and carries the explicit interpreter aggregate-carrier debt; `loans_tests::exclusive_reference_loop_elements_are_affine` proves a loop element is moved, not copied. The remaining affine matrix is open. |
