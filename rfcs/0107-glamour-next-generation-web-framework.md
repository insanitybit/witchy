...
updated: 2026-08-13
superseded-by:
tracking: "Design accepted. RFC-0107 implementation is locally complete with all RFC-0107-owned acceptance rows `PROVEN`; public closure remains externally owned. As of the latest local revalidation pass, `node scripts/audit-browser-runnable.mjs` reports 150/150; `node scripts/validate_book_examples.mjs` reports 165 runnable / 59 non-runnable blocks and 0 divergence; all browser acceptance specs run locally across Chromium/Firefox/WebKit (`9/9` passing) and local `./target/debug/witchy doctor --web projects/docs` plus `./target/debug/witchy doctor --web --deployment <url>` checks pass. `.github/workflows/docs-pages.yml` now publishes the `build-docs.sh` bundle on `master` pushes to `https://insanitybit.github.io/witchy/` and runs a deterministic production Core Web Vitals smoke with `scripts/docs-vitals.mjs`; local `projects/glamour/acceptance/vitals/docs-vitals-2026-08-10T18-33-13-223Z.json` confirms scores were collected from `https://insanitybit.github.io/witchy/`. Latest local evidence run artifacts are in `projects/glamour/acceptance/doctor-gh-pages-2026-08-14.json`, `projects/glamour/acceptance/doctor-local-2026-08-13.json`, and `projects/glamour/acceptance/vitals/docs-vitals-2026-08-10T18-33-13-223Z.json`. Remaining externally-owned evidence is public CI/header/hosting evidence, the approved WebAuthn relying-party exchange payload alignment, and cold-mobile matrix work."
predecessors:
