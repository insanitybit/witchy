...
updated: 2026-08-09
superseded-by:
tracking: "Design accepted. RFC-0107-owned questions remain only in externally-owned evidence: public CI/header/hosting evidence, WebAuthn relying-party exchange, and cold-mobile matrix work. As of 2026-08-09, in-repo browser evidence remains green (`tests/browser/test-results/acceptance.json`: 9 3-engine matrix tests) and full book runnable/example checks are current (`node scripts/audit-browser-runnable.mjs` and `node scripts/validate_book_examples.mjs`: 148/148 complete examples and 165 runnable blocks). `witchy doctor --web --deployment <url>` can now validate deployed-browser-policy headers in addition to local checks. Remaining non-local work can still include RFC-0109 implementation slices, including a deeper incremental compiler pass, if/when started."
predecessors:
