...
updated: 2026-08-12
superseded-by:
tracking: "Design accepted. RFC-0107 implementation is locally complete with all RFC-0107-owned acceptance rows `PROVEN`; public closure remains externally owned. As of the latest local revalidation pass (2026-08-12), `node scripts/audit-browser-runnable.mjs` reports 150/150; `node scripts/validate_book_examples.mjs` reports 165 runnable / 59 non-runnable blocks and 0 divergence; all browser acceptance specs run locally across Chromium/Firefox/WebKit (`9/9` passing) and local `./target/debug/witchy doctor --web projects/docs` plus `./target/debug/witchy doctor --web --deployment <url>` checks pass. `.github/workflows/docs-pages.yml` now publishes the `build-docs.sh` bundle on `master` pushes to `https://insanitybit.github.io/witchy/` to make deployed-header and release evidence collection a deterministic external step. Latest local evidence run artifacts are in `projects/glamour/acceptance/doctor-gh-pages-2026-08-12.json` and `projects/glamour/acceptance/doctor-local-2026-08-12.json`. Remaining externally-owned evidence is public CI/header/hosting evidence, the approved WebAuthn relying-party exchange payload alignment, and cold-mobile matrix work."
predecessors:
