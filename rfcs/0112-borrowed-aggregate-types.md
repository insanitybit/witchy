...
created: 2026-08-01
updated: 2026-08-09
tracking: "Design accepted; no open questions (non-goals fixed: no mutable/shared refs, no pointer identity, no loan across await/yield initially). Adds lifetime parameters to nominal type declarations; shared read-only borrows only. Criteria 1-5 PROVEN; criteria 6-11 PARTIAL. Row 6 transports exact before/after root sets through scalar updates and declared borrowed-field replacement. Row 8 has explicit-return, branch, loop, and ? cleanup coverage but lacks checked-heap/UAF evidence. Row 9 supports direct List(B('a)) construction, copy, list.at reads, and for traversal with checked hidden root companions, but lacks overwrite, drop, nesting, and erasure coverage. Row 10 has a compiled-Wasm parser/iterator shell fixture but still lacks an interpreter differential and zero-materialization counters. Row 11 documents and reflects the partial contract but lacks generated user docs and runnable zero-copy counters. Remaining before implemented: owned-companion materialization (row 7), aggregate checked-heap/UAF evidence (row 8), the remainder of List(B('a)) lifecycle (row 9), parser/iterator differential plus zero-materialization counters (row 10), and the remaining generated-contract evidence (row 11). Rust-class scalar performance is the long-term target: accepted opt-path borrows add no dynamic checker and no payload materialization, while the stable point-based facts permit the solver and lowering to be replaced or specialized when corpus evidence warrants it. Per rfcs/0110-0112-acceptance-ledger.md."
predecessors:
