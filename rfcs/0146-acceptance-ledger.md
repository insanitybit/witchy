...
| 1 awaited select fusion | partial infrastructure: 1B + ready-path 1C + pure-lane proof + typed Int payload projection + direct typed decode + reusable scheduler frame ABI; compiler-owned allocation-free emitter remains open | schema-1 prerequisite artifact `/Users/cobrien/.local/share/witchy/evidence/rfc0146-track1-direct-decode-ea279f45/acceptance-schema1.json` (`sha256:48e3cc3b006f67a54d5d4393e82545ec83dbe9d0c6b7874042a842468f791f4d`) | carrier/WAT assertion, RFC-0129 channel contract, deterministic schedule parity, and dedicated frame-ABI gate; no zero-allocation counter proof yet | `select_fanin` 20.754 to 12.183 ms median (41.30% faster) prerequisite; frame substrate sanity artifact `/Users/cobrien/.local/share/witchy/evidence/rfc0146/track1-frame-ade6edb0/` is not promotion evidence; `chan_throughput` remains unchanged in the prerequisite control | `impl/rfc0146-track1-frame2`, source `c7b85955`; no promotion claim | frame substrate merged as `0838346fb643871499d968382d37ee7e6e370913` at `2026-08-24T00:24:03Z`; journal line 8701 and gate log `state/merge-queue/logs/20260823-201941-impl~rfc0146-track1-frame2-74028-76.log`; Track 1 remains open |
...
Commits `ea279f45` and `36eee7b8` change only the fused `chan.__select2_map`
decoder and its formatter-safe helper shape. Instead
...
The merged master was rebuilt and rerun through the full correctness sweep;
all seventeen result hashes matched. A post-merge full timing artifact was
also generated, but system contention produced outliers and it is retained as
a sanity run rather than promotion evidence. The durable paired artifact above
remains the measured prerequisite evidence.
...
The current-master follow-up used source `7d70f4bcd2636b6d0c176507b399f4864408220a` and release binary `target-track5-current/release/witchy` (`sha256:67563a1a7a9389932151c2c3044b7e39b7faecbfb652f4a0ae76f50ffadc6406`). A temporary fixture executed `fib(35)` 96 times inside one monotonic-clock region, produced checksum `885836640`, and measured approximately 2.83 s of kernel time. Samply ran with `--main-thread-only --jit-markers --unstable-presymbolicate`; the durable profile is `/Users/cobrien/.local/share/witchy/evidence/rfc0146/track5-current-67563a1a/fib-current.samply.json.gz` (`sha256:5c8972a6fa2608f077bfc8bc0d83467bdc502e111dfe6cf8dca7242036fd99f0`) with sidecar `fib-current.samply.json.syms.json` (`sha256:8e5f9df4cc898d4cb376ec05bb7bc08d1a57bb25f6cdb3d1c86cdf3bd4a89019`). It remains `symbolicated=false`; the sidecar contains no fib or Wasm-JIT symbols. A second `WITCHY_PROFILE=perfmap` capture is retained (`sha256:8dd2c4e67246d10b3a116a3760812fb416f27a71e1b4fe92efc87b032dbadd3d`), but has no usable attributable JIT samples. Direct probes recorded that Wasmtime `jitdump`, VTune, and Pulley are unsupported on this macOS build, while `xctrace` cannot run without full Xcode. The longer fixture and JIT-marker attempt strengthen the environmental rejection, but do not establish the >=10% recursive call/return entry criterion; Track 5 remains deferred pending a Linux/Wasmtime symbolized profile or equivalent attribution.
