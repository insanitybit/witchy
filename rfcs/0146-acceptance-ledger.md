...
| 1 awaited select fusion | partial infrastructure: 1B plus ready-path 1C landed; allocation-free 1D open | `/tmp/rfc0146-track1-1b-20260823.tsv`; fresh ready-path spot pair: candidate 19.18/19.50/18.91 ms, master 22.40/23.21/22.86 ms | WIR/carrier fusion test; ready-path WAT and semantic output evidence; no allocation-free counters yet | syntax fusion/deopt, WIR plan, carrier tests, select semantic output/parity passed | ready-path spot samples remain comparable to 1B; no promotion claim | `impl/rfc0146-track1-1cd`, landed as `730426f9` (source commit `c80e8e8b`); gate baseline `de55aa86` | merged through queue; Track 1 remains open pending 1D |
...
| 3 WIR inlining | rejected: scalar-leaf experiment had no reliable whole-workload gain | current-master paired baseline retained in Track 3 worktree; candidate WAT proves the hot closure call was removed | `witchy-wir` native 129 tests passed; positive/negative hygiene and envelope tests passed | `closure_calls` regressed 7.692 to 8.967 ms; `expr_eval` was flat 11.565 to 11.570 ms; `binary_trees` improved only 56.339 to 55.307 ms; RFC requires closure_calls >=20% and protected rows <=5% | `perf/rfc0146-inline`; experiment not queued | no terminal queue event; do not queue without a reproducible whole-workload gain |
...
On current master `7864f03e`, the existing `recursive_inline` module was found to be a no-op for `fib`: its single-expression extractor did not admit the value-producing `If`, and its `target.locals.is_empty()` gate rejected the lowerer's predeclared scratch-local catalog (`fib.fib` reports 403 catalog locals even though the generated WAT has no local declarations). A detached experiment retained the strict scalar/no-loop/no-host/no-reference filters, admitted only bodies with no non-parameter local reads, and cloned a value-producing `If` as a typed `WirExpr::Control`. Focused `witchy-wir` checks passed (42 tests), and the candidate WAT changed from three `call $fib.fib` sites to five, proving one-level cloning fired with no ABI change.
