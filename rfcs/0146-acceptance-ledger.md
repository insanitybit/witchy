...
| 1 awaited select fusion | partial infrastructure: 1B landed on branch; 1C/1D open | `/tmp/rfc0146-track1-1b-20260823.tsv` (12 paired kernel samples; source and binary identities in header) | generated WIR/carrier fusion test; no scalar-emitter counters yet | syntax fusion/deopt, WIR plan, carrier tests passed; protected 12-sample matrix passed correctness | `select_fanin` 20.223 vs 22.925 ms (`0.882x`); controls remained within expected variance | `impl/rfc0146-track1-abi`, 1B `a995eadc` | pending review; not Track 1 acceptance |
...
| 5 recursive inlining | deferred: entry criterion not established | `state/agents/rfc0146-track5/fib-baseline.tsv` (`sha256:9036f6523a5285fb24e1c1d73f260d0cbc85c84151c0cb9e71bf44fec4eabdcf`) | `fib-profile.json.gz` (`sha256:b29fe41dfb4da260f97f8610f2f67795cab5850912961891cf1cfda616cf58a4`) was unsymbolicated/zero-sample; WAT shows minimal scalar body with two recursive calls and no adapters/allocations | RFC requires native attribution of >=10% call/return overhead before implementation; no promotion evidence | `perf/rfc0146-recursive-inline`; no implementation changes | no terminal queue event; re-profile with a longer attributable fixture before reopening |
