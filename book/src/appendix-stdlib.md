...
| `cmp` / `show` | the comparison hierarchy (`PartialEq`/`Eq`/`PartialOrd`/`Ord`, backing `== != < > <= >=`) and display trait, plus generic algorithms (`cmp.sort`, `cmp.max_of`, …); list membership/lookup live in `list` (`list.contains`, `list.index_of`) |
